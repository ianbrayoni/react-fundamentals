...
- `id` helps focus the input, `name` doesn't
- Calling functions in react does not require `()` e.g `{handleSubmit}` is already a callable.
- `event.preventDefault()` prevents the default behavior of a page refresh
- Good practice to make things explicit e.g `{Boolean(error)}` instead of `{error}`
- `isLowerCase` is a better name compared to `isValid`
- I learnt how to specify inline styles as an expression in JSX: `style={{color: 'red'}}`. Refer to Lesson 5.
...
Fill out
[the feedback form](https://ws.kcd.im/?ws=React%20Fundamentals%20%E2%9A%9B&e=06%3A%20Forms&em=brayoni.ian%40gmail.com).
